...
export class PDFProcessingService {
  ...
  static async processPDF(fileUrl: string): Promise<ProcessingResult> {
    try {
      ...
      console.log(`Successfully processed PDF: ${chunks.length} chunks created`);
  ...
  private static chunkText(text: string): ProcessedChunk[] {
    ...
    for (const sentence of sentences) {
      ...
      if (currentSize + sentenceWithPunctuation.length > this.CHUNK_SIZE && currentChunk.length > 0) {
        chunks.push({
        ...
        const overlapWords = words.slice(-Math.floor(this.CHUNK_OVERLAP / 6)); // Approximate overlap
   ...
   */
  static async generateEmbeddings(chunks: ProcessedChunk[]): Promise<{ embeddings: number[][]; success: boolean; error?: string }> {
    try {
