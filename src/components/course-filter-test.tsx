    ...
    return (
        <div className="p-4 border rounded-lg bg-gray-50">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    <label className="block text-sm font-medium mb-2">
                        Current Semester: {filterSemester}
                    </label>
                    <select
                        value={filterSemester}
                        onChange={(e) =>
                            setFilterSemester(Number(e.target.value) as 1 | 2)
                        }
                        className="w-full p-2 border rounded"
                    >
            ...
            <div className="mb-4">
                <h4 className="font-medium mb-2">
                    All Courses ({coursesResponse?.courses?.length || 0})
                </h4>
                <div className="text-xs max-h-32 overflow-y-auto">
                    {coursesResponse?.courses?.map((course) => (
                        <div key={course.id} className="mb-1">
                            {course.code} - Level {course.level}, Semester{" "}
                            {course.semester}
                        </div>
                    ))}
                </div>
            ...
            <div>
                ...
                <div className="text-xs max-h-32 overflow-y-auto">
                    {filteredCourses.map((course) => (
                        <div
                            key={course.id}
                            className="mb-1 bg-green-100 p-1 rounded"
                        >
                            {course.code} - Level {course.level}, Semester{" "}
                            {course.semester}
                        </div>
