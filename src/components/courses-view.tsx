    ...
    <div className="space-y-6">
      ...
      >
        ...
        <TabsContent value="1" className="mt-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold">First Semester Courses</h2>
              <p className="text-sm text-muted-foreground">
                {coursesBySemester.semester1.length} course{coursesBySemester.semester1.length !== 1 ? 's' : ''}
              </p>
            </div>

            {coursesBySemester.semester1.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-muted-foreground">No courses available for first semester</p>
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {coursesBySemester.semester1.map((course: Course) => (
                  <CourseCard
                    key={course.id}
                    course={course}
        ...
        <TabsContent value="2" className="mt-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold">Second Semester Courses</h2>
              <p className="text-sm text-muted-foreground">
                {coursesBySemester.semester2.length} course{coursesBySemester.semester2.length !== 1 ? 's' : ''}
              </p>
            </div>

            {coursesBySemester.semester2.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-muted-foreground">No courses available for second semester</p>
              </div>
            ) : (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {coursesBySemester.semester2.map((course: Course) => (
                  <CourseCard
                    key={course.id}
                    course={course}
                    onViewDetails={handleViewCourse}
